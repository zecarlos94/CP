...
\\
\>[B]{}\mbox{\onelinecomment import TLTree}{}\<[E]%
\\
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{render}\;\Varid{html}\mathrel{=}\mathbf{do}\;\{\mskip1.5mu \Varid{writeFile}\;\text{\tt \char34 sierpinski.html\char34}\;\Varid{html};\Varid{system}\;\text{\tt \char34 open~sierpinski.html\char34}\mskip1.5mu\}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\\
\>[6]{}\hsindent{3}{}\<[9]%
\>[9]{}\Varid{execExempSeq}\mathrel{=}\underline{(\Varid{putStrLn}\comp \Varid{show}\comp \Varid{runEval}\comp (\Varid{parBTreeMap}\;\Varid{fib})\mathbin{\$}\Varid{t1})}{}\<[E]%
\\
\>[6]{}\hsindent{3}{}\<[9]%
\>[9]{}\Varid{execExempPar}\mathrel{=}\underline{(\Varid{putStrLn}\comp \Varid{show}\comp (\Varid{fmap}\;\Varid{fib})\mathbin{\$}\Varid{t1})}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\\
\>[B]{}\Varid{execExerPar}\mathrel{=}\underline{(\Varid{putStrLn}\comp \Varid{show}\comp \Varid{runEval}\comp (\Varid{parBTreeMap}\;\Varid{fib})\mathbin{\$}\Varid{t1})}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{execExerSeq}\mathbin{::}[\mskip1.5mu \Conid{String}\mskip1.5mu]\to \Conid{IO}\;(){}\<[E]%
\\
\>[B]{}\Varid{execExerSeq}\mathrel{=}\underline{(\Varid{putStrLn}\comp \Varid{show}\comp (\Varid{fmap}\;\Varid{fib})\mathbin{\$}\Varid{t1})}{}\<[E]%
\\[\blanklineskip]%
...
A função \ensuremath{\Varid{depth}} recebe uma LTree e devolve um inteiro com o fator de balanceamento da árvore. Neste caso, usamos um catamorfismo de um \ensuremath{\alt{\Varid{zero}}{\Varid{succ}\comp \uncurry{\Varid{max}}}}, em que \ensuremath{\Varid{zero}} é o fator de balanceamento da árvore que contém só uma folha e \ensuremath{\Varid{succ}} . \ensuremath{\uncurry{\Varid{max}}} caso a árvore contenha mais que uma folha e, nessa situação, o balanceamento é calculado através do sucessor do maior fator de balanceamento entre as árvores da esquerda e da direita.

A função \ensuremath{\Varid{balance}} contém uma função \ensuremath{\Varid{tips}}, que coloca todos os elementos de uma LTree numa lista com todos os elementos da mesma. Essa lista vai, depois, ser organizada pelo anamorfismo da função \ensuremath{\Varid{lsplit}}, que vai tratar de organizar a mesma, para depois devolver uma nova árvore, desta vez, balanceada, consoante a disposição dos elementos na lista.
...
A função \ensuremath{\Varid{qsplit}} corresponde a um gene e funciona como função auxiliar da \ensuremath{\Varid{abpe}}. Recebe como argumento um par de inteiros, que corresponde ao intervalo em que a árvore vai ser definida, e pega no elemento médio do mesmo (através da função \ensuremath{\Varid{med}}) e coloca-o no topo a BTree, e de seguida, gera dois intervalos, que correspondem aos intervalos de valores da árvore, que se situam antes e depois do valor intermédio.
...
\\
\>[B]{}\Varid{qsplit}\;(\Varid{x},\Varid{y})\mathrel{=}\mathbf{if}\;(\Varid{x}\mathbin{>}\Varid{y})\mathrel{\vee}(\Varid{x}\equiv \mathrm{0}\mathrel{\wedge}\Varid{y}\equiv \mathrm{0})\;\mathbf{then}\;i_1\;()\;\mathbf{else}\;i_2\;(\Varid{med},((\Varid{x},\Varid{med}\mathbin{-}\mathrm{1}),(\Varid{med}\mathbin{+}\mathrm{1},\Varid{y}))){}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\mathbf{where}\;\Varid{med}\mathrel{=}\Varid{x}\mathbin{+}\Varid{y}\div \mathrm{2}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\subsection*{Secção \ref{sec:SList}}

Nesta secção estão presentes várias funções para a biblioteca SList, tal como estão definidas para as restantes bibliotecas. Contém também uma função \ensuremath{\Varid{mgen}}, que é o gene de \ensuremath{\Varid{merge'}}. Recebe um par de listas e parte-a, separando a cabeça da primeira lista do par com a sua cauda e com a segunda lista recebida como parâmetro. 
...
\\
\>[B]{}\Varid{outSList}\;(\Conid{Sent}\;\Varid{b})\mathrel{=}i_1\;(\Varid{b}){}\<[E]%
\\
\>[B]{}\Varid{outSList}\;(\Conid{Cons}\;(\Varid{a},\Varid{b}))\mathrel{=}i_2\;(\Varid{a},\Varid{b}){}\<[E]%
\\[\blanklineskip]%
...
Nesta secção e, tal como na anterior, estão presentes funções para uma biblioteca, neste caso a TLTree. A função \ensuremath{\Varid{geraSierp}} gera um triângulo de Sierpinski através de um anamorfismo, cujo gene, a função \ensuremath{\Varid{geneSierp}} divide a estrutura do tipo Tri em três partes, para assim formar o triângulo, diminuindo o nível de recursividade em uma unidade.

A função \ensuremath{\Varid{apresentaSierp}} recebe uma estrutura do tipo TLTree e trasforma-a numa lista de estruturas do tipo Tri. Faz exatamente o mesmo que a função tipsTLTree, pelo que a sua presença é redundante.

A função \ensuremath{\Varid{rep}} transforma por fim a estrutura TLTree num triângulo de Sierpinski, que pode posteriormente ser visto numa página web, no ficheiro "sierpinski.html". A função começa por gerar a estrutura do triângulo, chamando a função \ensuremath{\Varid{geraSierp}} que, posteriormente, deverá sofrer um \ensuremath{\Varid{uncurryTLTree}} para ser adaptado e poder ser recebido como input da \ensuremath{\Varid{apresentaSierp}}, que vai transformar a TLTree numa lista de Tri's. Por fim, cada elemento da lista vai ser passado para uma String própria para ficheiros html pela função \ensuremath{\Varid{drawTriangle}} e a função \ensuremath{\Varid{finalize}} faz o acabamento final, para que seja possível o ficheiro html ser aberto numa página web e o triângulo possa ser visualizado.
...
\\
\>[B]{}\Varid{outTLTree}\;(\Conid{N}\;(\Varid{t1},(\Varid{t2},\Varid{t3})))\mathrel{=}i_2\;(\Varid{t1},(\Varid{t2},\Varid{t3})){}\<[E]%
\\[\blanklineskip]%
...
\\
\>[B]{}\Varid{apresentaSierp}\mathrel{=}\Varid{tipsTLTree}{}\<[E]%
\\[\blanklineskip]%
...
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}\Varid{html}\mathrel{=}\Varid{rep}\;\Varid{dados}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{rep}\mathrel{=}\Varid{finalize}\comp \Varid{concat}\comp (\Varid{map}\;\Varid{drawTriangle})\comp \Varid{apresentaSierp}\comp (\Varid{uncurryTLTree}\;\Varid{geraSierp})\comp \Varid{split}\;\p1\;\p2{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
...
A função \ensuremath{\Varid{gene}} devolve um \ensuremath{\alt{\Varid{stop}}{\Varid{perder}}}, em que retorna a probabilidade de a última palavra, ou seja, a palavra "stop", se perder e também a probabilidade de qualquer outra palavra da lista ter o mesmo destino. Funciona como função auxiliar da \ensuremath{\Varid{transmitir}}.

A função \ensuremath{\Varid{perder}} calcula a probabilidade qualquer palavra da lista se perder, exceto a palavra "stop", cuja probabilidade de desaparecimento vai ser calculada pela função \ensuremath{\Varid{stop}}.

\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{gene}\mathrel{=}\alt{\Varid{stop}}{\Varid{perder}}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{perder}\;(\Varid{a},\Varid{b})\mathrel{=}\Conid{D}\;[\mskip1.5mu ((\Varid{a}\mathbin{:}\Varid{b}),\mathrm{0.95}),(\Varid{b},\mathrm{0.05})\mskip1.5mu]{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{stop}\mathrel{=}\underline{(\Conid{D}\;[\mskip1.5mu ([\mskip1.5mu \mskip1.5mu],\mathrm{0.10}),([\mskip1.5mu \text{\tt \char34 stop\char34}\mskip1.5mu],\mathrm{0.90})\mskip1.5mu])}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks

A probabilidade de a palavra "atacar" se perder é de \ensuremath{\mathrm{4.1}\mathbin{\%}}.

A probabilidade de chegarem todas as palavras, mas faltar o "stop" no fim, é de \ensuremath{\mathrm{8.6}\mathbin{\%}}.

A probabilidade de a transmissão ser perfeita é de \ensuremath{\mathrm{77.2}\mathbin{\%}}.

\subsection*{Secção \ref{sec:parBTreeMap}}

A função \ensuremath{\Varid{parBTreeMap}} aplica a função \ensuremath{\Varid{rpar}} a todos os elementos de uma BTree.
